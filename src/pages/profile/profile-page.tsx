...
import type { UserProfileType } from "@store/features/user-slice/user-types";
import Preloader from "@components/preloader/preloader";
import {
  ProfileView,
  type ProfileUser,
} from "@components/profile-view/profile-view";

import ava1 from "@assets/images/profile/ava-1.png";
import styles from "./profile-page.module.scss";

// Преобразование данных из API в локальный формат
const mapProfileToLocalUser = (
...
export const ProfilePage = () => {
  ...
  const { currentUserProfile, loading, error, companyStructure } = useSelector(
    (state: RootState) => state.user,
  );
  const { user: authUser } = useSelector((state: RootState) => state.auth);

  const [user, setUser] = useState<ProfileUser>(() =>
    mapProfileToLocalUser(currentUserProfile),
  );
